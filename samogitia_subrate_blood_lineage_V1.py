...
#In this lineage tracing version version, working to incorporate unique branch info from Gytis.
        ...
        if treecount==0: ## At tree state 0 insert header into output file
            ...
            if lower==0 and upper==np.inf: ## only add a header if not doing a chunk
                ...
                if 'subrate' in analyses:
                    ...
                    outfile.write('\t%s\tCombo\tOther\tFull'%('\t'.join(sorted(subgroups.keys()))))
...
#                    print treecount, lensubs[g]
                ...
                for g,n in nodes_and_leaves.iteritems():
                    ...
                    #lensubs_want[g] = lineage_trace_info(tMRCA_node, leaves_we_want)
                    lineage_trace = []
                    lensubs_want[g] = lineage_trace_info(n[0], n[1])
                    
                    #print lensubs_want[g][0]
                    #print lensubs_want[g][1]
                    #print lensubs_want[g][1] / lensubs_want[g][0]
                ...
                #Calc average rate combining branches from all subgroups
                #!!!! If any of the subgroups are overlapping, shared branches will be double counted and therefore given higher weight
                comboSubgroupsRate = sum([x[1] for x in lensubs_want.values()])/sum([x[0] for x in lensubs_want.values()])
                ...
                restRate = (fullSubs - sum([x[1] for x in lensubs_want.values()]))/(fullLen - sum([x[0] for x in lensubs_want.values()]))
                outfile.write('\t%s\t%.6f\t%.6f\t%.6f'%('\t'.join(outSubrate), comboSubgroupsRate, restRate, fullSubs/fullLen))
            ###################################################
